...
void calculate_depth(InputArray _normal, OutputArray _depth) {
	...
	Mat depth(grd_y.rows, grd_y.cols, CV_32FC1, Scalar(0));
	const auto ctr_h = grd_y.rows >> 1;
	const auto ctr_w = grd_y.cols >> 1;

	// init value
	depth.at<float>(ctr_h, 0) = 0;
	for (int j = ctr_w + 1; j < grd_y.cols; j++) {
		depth.at<float>(ctr_h, j) = depth.at<float>(ctr_h, j - 1) + grd_x.at<float>(ctr_h, j);
	}

	auto range_east = make_range(ctr_w, grd_y.cols);
	auto range_west = make_range(ctr_w - 1, -1, -1);

	auto range_south = make_range(ctr_h, grd_y.rows);
	auto range_north = make_range(ctr_h - 1, -1, -1);
